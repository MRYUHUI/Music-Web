...
const HttpManager = {
  ...
  // =======================> 收藏列表 API 完成
  // 返回的指定用户ID收藏列表
  getCollectionOfUser: (userId) => get(`user_like/detail?userId=${userId}`),
  // 删除收藏的歌曲
  deleteCollection: (userId, songId) => deletes(`user_like/delete?userId=${userId}&&songId=${songId}`),
  ...
  // 更新歌手信息
  updateSingerMsg: ({ id, name, sex, birth, location, introduction, pic }) => post(`singer/update`, {
    id,
    name,
    sex,
    birth,
