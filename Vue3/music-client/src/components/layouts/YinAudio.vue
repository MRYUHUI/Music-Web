...
export default defineComponent({
  setup() {
    ...
    // 这个函数其实是一个箭头函数，它接收一个参数 el，
    // 这个 el 就是当前的 DOM 元素。这段代码的作用是将 el
    // 赋值给 divRef.value，从而将 divRef 和 <audio>
    // 元素关联起来。这样，在之后的代码中，
    // 通过 divRef.value 就可以访问到这个 <audio> 元素了。
    ...
    // const isLoopPlay = computed(() => store.getters.isLoopPlay) // 判断是否是循环播放
    const rePlayThisSong = computed(() => store.getters.rePlayThisSong)

    // 监听播放还是暂停，在 bar 里点击播放以后，更改isPlay的值，这里就会监听到
