...
export default defineComponent({
  ...
  setup() {
    ...
    const userLikeSongList = ref([]) // 收藏的歌曲
    ...
    // 获取收藏的歌曲
    async function getUserLike(userId) {
      userLikeSongList.value = []
      const result = (await HttpManager.getUserLikeByUserId(
        userId
      )) as ResponseBody
      console.log(result.data)
      const userLikeIDList = result.data || [] // 存放收藏的歌曲ID
      // 通过歌曲ID获取歌曲信息
      for (const item of userLikeIDList) {
        if (!item.songId) {
        ...
        userLikeSongList.value.push(result.data)
      }
    }

    function changeData() {
      getUserLike(userId.value)
    }
