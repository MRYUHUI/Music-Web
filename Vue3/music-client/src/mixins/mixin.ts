...
export default function () {
  ...
  function checkStatus(status?: boolean) {
	...
	// 更新历史记录
	const updateHistory= async (userId, songId)=> {
		await HttpManager.updateHistory({userId, songId})
	}
  // 播放
  const playMusic = async ({ id, url, pic, index, name, lyric, currentSongList }) =>{
    ...
    proxy.$store.dispatch("playMusic", {
      ...
      currentSongList,
		...
		if(token.value){
			await updateHistory(store.getters.userId, id)
		}
  }
